...
public abstract class TempPageHolder {
  ...
  private static final SoftReference EMPTY_BUFFER_REF =
...
//    @Override
  ...
  {
    private SoftReference _buffer = EMPTY_BUFFER_REF;

//    @Override
    protected ByteBuffer getExistingBuffer() {
      return (ByteBuffer)(_buffer.get());
    }
    
//    @Override
    protected ByteBuffer getBuffer(PageChannel pageChannel) {
      ByteBuffer buffer = (ByteBuffer)(_buffer.get());
      if(buffer == null) {
        buffer = pageChannel.createPageBuffer();
        _buffer = new SoftReference(buffer);
      }
      return buffer;
    }
