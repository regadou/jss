...
public class Interpreter extends ImporterTopLevel
{
	...
	{"read", "write", "print", "log", "exit", "load", "pwd", "login", "crypt", "parameter", "parse", "interactive",
	...
	public static void main(String[] args)
	{
		...
		catch (Exception e) { getException(e, true); }
		finally { Interpreter.terminate(js); }
	}
   ...
   public static Object[] getArray(Scriptable obj)
   {
	...
	public static long parseDate(String txt)
	{
		if (txt == null || txt.equals(""))
			return (new Date()).getTime();
		try { return (new Date(Date.parse(txt))).getTime(); }
		catch (Exception e)
		{
	...
	public Object load(Object src)
	{
		...
		if (src instanceof String)
		{
			...
			if (first != '/' && first != '\\' && txt.charAt(1) != ':')
			{
				...
				{
					try { dir = (new File("./")).getCanonicalPath()+File.separator; }
					catch (Exception e) { throw new RuntimeException(e.toString()); }
				}
				src = dir+txt;
				txt = src.toString();
			...
			if (p >= 0 || p2 >= 0)
			{
				if (p < 0 || p2 > p)
					p = p2;
				String path = null;
				try { path = (new File(txt.substring(0,p+1))).getCanonicalPath()+File.separator; }
				catch (Exception e) { throw new RuntimeException(e.toString()); }
				directory.add(path);
				diradd = true;
			}
...
*/
	...
	public String pwd()
	{
		...
		{
			try { return (new File("./")).getCanonicalPath()+File.separator; }
	...
	public String parameter(String name)
	{
		...
		else if (servlet == null)
		{
			...
			Map map = Servlet.loadParameters(pwd());
			Object node = Servlet.getMap(map, "/web-app/servlet/init-param");
			List lst;
...
write("parameters="+parameters+"<br>\n");
   ...
   public Servlet getServlet()
   {
	...
	public static void terminate(Interpreter js)
	{
		...
		for (int i = 0; i < regs.length; i++)
		{
			...
			try
			{
				java.lang.reflect.Method fn = obj.getClass().getMethod("close", null);
				if (fn != null)
					fn.invoke(obj, null);
			}
			catch (Exception e) {}
