...
public class Servlet extends HttpServlet {
   ...
   private Map application = null, threads = null, realms = null;
   ...
   public static Object getUploads() {
      return uploads;
   }
   ...
   public void init() throws javax.servlet.ServletException {
      ...
      if (param != null && !param.equals("")) {
         try {
            WAITTIME = Integer.parseInt(param);
         } catch (Exception e) {
         }
      }
      ...
      param = getInitParameter("debug");
      if (param != null && param.trim().toLowerCase().equals("true")) {
         debug = true;
      }
   }
   ...
   public void doRequest(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
      ...
      try {
         ...
         if (file.isDirectory()) {
            ...
            validateInput(file.toString(), js);
   ...
   private void validateInput(String filename, Scriptable global) {
      String[] parts = filename.split("/");
      Object appValidation = getValue(application, "validation");
      Object validation = getValue(appValidation, parts[parts.length-1]);
      if (validation == null) {
         if (parts.length > 1)
            validation = getValue(appValidation, parts[parts.length-2]+"/"+parts[parts.length-1]);
         if (validation == null)
            return;
      }
      for (Object key : getKeys(validation)) {
         Object rule = getValue(validation, key);
         if (rule == null)
            continue;
         Object value = getValue(global, key);
         if (!validateRule(rule, (value == null) ? null : value.toString()))
            throw new RuntimeException("Validation for "+key+" in "+filename+" failed: "+value);
      }
   }
   
   private boolean validateRule(Object rule, String value) {
      if (rule == null)
         return true;
      Object[] tokens;
      if (rule instanceof Collection)
         tokens = ((Collection)rule).toArray();
      else if (rule instanceof Object[])
         tokens = (Object[])rule;
      else
         tokens = rule.toString().split(",");
      for (Object token : tokens) {
         if (token == null)
            token = "null";
         String criterion = token.toString();
         switch (criterion) {
            case "int":
               try {
            ...
            case "null":
               ...
               break;
            default:
               int maxlength = Integer.parseInt(criterion);
   ...
   private Object getValue(Object parent, Object key) {
      Object value;
      if (parent instanceof Scriptable)
         value = ((Scriptable)parent).get(key.toString(), (Scriptable)parent);
      else if (parent instanceof Map)
         value = ((Map)parent).get(key);
      else if (parent instanceof Collection)
         value = ((Collection)parent).toArray()[Integer.parseInt(key.toString())];
      else if (parent instanceof Object[])
         value = ((Object[])parent)[Integer.parseInt(key.toString())];
      else
         return null;
      if (value == Context.getUndefinedValue() || value instanceof UniqueTag)
         return null;
      return value;
   }
   
   private Object[] getKeys(Object parent) {
      if (parent instanceof Scriptable)
         return ((Scriptable)parent).getIds();
      if (parent instanceof Map)
         return ((Map)parent).keySet().toArray();
      
      int length;
      if (parent instanceof Collection)
         length = ((Collection)parent).size();
      else if (parent instanceof Object[])
   ...
   /**
    * ******************
    * This function will return a realm object which should contain the
    * following fields: file: the source file where to get the user list or null
    * if none realm: the realm name crypted: true if passwords are crypted else
    * false (text values) time: a long value representing seconds since january
    * first 1970 users: the list of valid users, which each being a {user,pass}
    * map object
    *
    ********************
    */
